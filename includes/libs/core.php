...
/**
 *
 * Conjunto de clases desarrolladas para Arregla Sanse
 *
 ...
 * @license https://github.com/alhgo/arreglasanse/blob/master/LICENSE
 */
	...
	public function userInsertTemp($data,$mail=true)
	{
		...
		else
		{
			...
			else if($mail == true && isset($data['token']))
			{
				...
				$body->bodyPreheader = "Solicitud de registro en Arregla Sanse";
				...
				$body->bodyContent = "
									<p>Se ha recibido la solicitud para registrarse en Arregla Sanse. Visite el siguiente enlace para confirmar su cuenta de correo $url_confirm </p>
									" . $button . "
				...
				$mail->Subject  = 'Solicitud de registro en Arregla Sanse';
		...
		else
		{
			...
			else 
			{
				...
				if($mail == true)
				{
					...
					$body = new mailBody; 
					//Construimos el texto de previo para algunos clientes
					$body->bodyPreheader = "Confirmado registro de usuario en Arregla Sanse";
					$body->bodyContent = "
										<p>Se ha confirmado el registro de usuario en la aplicación <a href='" . c::get('site.url') . "'>Arregla Sanse</a>.</p>
					...
					$mail->Subject  = 'Registro de usuario confirmado en Arregla Sanse';
					$mail->isHTML(true); //Indicamos que es un correo HTML
					$mail->CharSet = 'UTF-8'; //Convertimos los caracteres a UTF8
					$mail->Body = $body->bodyHTML; //Insertamos el cuerpo construido previamente
					//Enviamos el correo y comprobamos que se ha mandado correctamente	
